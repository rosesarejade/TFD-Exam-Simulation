...
def windowed_dataset(series, batch_size, n_past=24, n_future=24, shift=1):
    ...
    dataset = dataset.flat_map(lambda w: w.batch(n_past+n_future))
    dataset = dataset.shuffle(buffer_size=10000)
    dataset = dataset.map(lambda w: (w[:-n_past], w[-n_past:,:1]))
    # YOUR CODE HERE
...
def solution_C5():
    ...
    model = tf.keras.models.Sequential([
        #tf.keras.layers.InputLayer((N_PAST, N_FEATURES)),
    ...
    class myCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            if (logs.get('mae') < 0.05):
                print('\nMean absolute error reached below 0.1! Stopping training...')
    ...
    # Code to predict the test set *-*
    '''test_predict = model.predict(valid_set).flatten()
    test_predict = test_predict[:len(x_valid)]
